...
export default function Search({foods,cats,restaurants}) {
    ...
    return (
        ...
        <div className='fixed top-0 flex justify-center w-[31vw] min-w-[18.75rem] m-auto py-spacing-16 mt-[.8rem]'>
            ...
            <div className='w-[95%] flex flex-col'>
                <div className='relative w-full flex items-center justify-center '>
                    ...
                    <input ref={INPUT} onChange={handleSearchChange} value={value} type="text" className='outline-none focus:border-black w-full  h-12 py-spacing-16 pl-spacing-16 pr-spacing-5 border-[0.0625rem] border-carbon-alphaHigh rounded-[0.625rem] font-vrg 
                ...
                <div className=' bg-white mt-[0.25rem] rounded-[0.375rem] overflow-y-auto max-h-[70vh]'>
                    ...
                    <div className={`${value.length>1? "hidden" : ""} px-[1.125rem] pb-[1.125rem] pt-[.8rem] cursor-pointer `}> 
{/* nothing */}
                      {!(value.length)&&<span className='font-vrg text-[0.875rem] leading-lineHeight-caption text-carbon-main'>
...
{/* length input one */}
                        ...
                        {(value.length === 1) &&<div className='flex justify-between'>
                    ...
                    {value.length>1&&cats.filter((item)=>{
                     ...
                     {value.length>1&&restaurants.filter((item)=>{
                    ...
                    }).map((item,index,arrayRef)=><SearchMarket resturantName={item.name} count={arrayRef.length}/>)}  
...
{/*                         
                    ...
                    return<SearchProduct foodName={item.name} img={item.image} price={item.price} count={arrayRef.length} />})}  */}
                     ...
                     {value.length>1&& <SearchProduct foods={foods} value={value} restaurants={restaurants}/>}        
